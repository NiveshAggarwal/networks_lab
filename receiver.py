...
class Receiver:
    def __init__(self, base):
        self.base= base
        self.diff = 400
        self.freq = np.arange(3000, 3000 + self.diff * (self.base+1) , self.diff)
    ...
    def calibrate(self, sample_rate: int = 44100, duration: float = 0.03):
        ...
        stream.close()
        audio.terminate()
    def check_threshold(self, freq_power: np.ndarray)-> int:
        """
        Check if the maximum power of the frequency is above a certain threshold.

        Parameters:
            freq_power (np.ndarray): Numpy array containing the power of each frequency
            threshold (float): Threshold value

        Returns:
            int: Index of the frequency with maximum power
        """
        threshold=0.0 #need to change
        if np.max(freq_power) >= threshold:
            return np.argmax(freq_power)
        return -1
    ...
    def decode_audio_to_bits(self, sample_rate: int = 44100, bit_duration: float = 0.3):
        ...
        while True:
            ...
            # index_max=self.check_threshold(freq_power)
